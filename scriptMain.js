...
function calculateValues() {
  const firstSkillId = parseInt(
    document.getElementById("5w1h-first-skill-id").value,
    10,
  );
  const secondSkillId = parseInt(
    document.getElementById("5w1h-second-skill-id").value,
    10,
  );
  let firstSkillLvl = parseInt(
    document.getElementById("5w1h-first-skill-lvl").value,
    10,
  );
  let secondSkillLvl = parseInt(
    document.getElementById("5w1h-second-skill-lvl").value,
    10,
  );
  const talismanRarity = parseInt(
    document.getElementById("5w1h-talisman-rarity").value,
    10,
  );
  const numberOfSlots = parseInt(
    document.getElementById("5w1h-number-of-slots").value,
    10,
  );
  ...
  const Nama = document.getElementById(
    "5w1h-talisman-rarity" + talismanRarity,
  ).textContent;
  ...
  if (firstSkillLvl || secondSkillLvl) {
    if (yyyyHex.length > 4) {
      ...
      output.innerHTML = `_C0 Custom ${Nama} [${numberOfSlots} Slot]<br />_L 0x2174${zzaaHex} 0x${yyyyHex}6501<br />_L 0x2174${zzbbHex} 0x0000${xxxxHex}`;
    }
  } else {
